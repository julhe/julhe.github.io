...
import { OrbitControls } from './OrbitControls.js.js';
import { GLTFLoader  } from './GLTFLoader.js.js';
...
function sdfShaderFS(){
    ...
    uniform bool _ShowUnfiltered, _RectFilter, _SideBySide, _ShowShowUnfiltered;

    #define inverseLerp(a,b,x) ((x-a)/(b-a))
    void main() {  
        vec2 textureSize = vec2(128.0, 128.0);
        vec2 screencoord = gl_FragCoord.xy / _ScreenSize;
        float rawSdf = texture2D( vSDFmask, vUv ).r + vBias;
        ...
        float areaDiameter;
        if(_RectFilter){
            vec2 texelCoverageXY = fwidth(vUv) * textureSize; // calculate the area under the pixel
            areaDiameter = saturate(min(texelCoverageXY.x, texelCoverageXY.y));  
        } else {              
            // calculate the determinant / parallelogram area under the pixel, and multiply it with the texture size
            // useing abs() because its possible to get a negative area
            // NOTE: for some reason I cant use determinant() in WebGL, so I do it manually...
            float areaUnderPixelSqr = abs(uvDDX.x * uvDDY.y - uvDDX.y * uvDDY.x); 
            areaUnderPixelSqr *= textureSize.x * textureSize.y; // scale up uv-space to texel-space
            areaDiameter = sqrt(areaUnderPixelSqr); //
...
renderer.setClearColor( 0x1f222a, 1.0 );
...
var sdfTexture = new THREE.TextureLoader().load( 'beegurbancreativeengerie.png' );
...
var sdfMaterial = new THREE.ShaderMaterial({  
    uniforms: {
        ...
        _RectFilter: {type: 'b', value: parameters.RectFilter},
...
const 
    ...
    dMode_Rect = 'Length by Rectangle', 
...
function animate() {
    ...
    switch(parameters.DisplayMode){
        ...
        break;
        case dMode_Rect:
            sdfMaterial.uniforms._RectFilter.value = true;
        break;
        case dMode_NoFilter:
...
window.onload = function() {
    ...
    let modeDropdown = gui.add(parameters, 'DisplayMode', [dMode_ParVsNoFilter, dMode_Par, dMode_Rect, dMode_NoFilter, dMode_ShowShowUnfiltered]);
